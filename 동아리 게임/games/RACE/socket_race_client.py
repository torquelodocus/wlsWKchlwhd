...
def main():
    ...
    img_title = pygame.image.load(rsrc("image_race/title.png")).convert_alpha()
    img_bg = pygame.image.load(rsrc("image_race/bg.png")).convert()
    img_sea = pygame.image.load(rsrc("image_race/sea.png")).convert_alpha()
    img_obj = [
        None,
        pygame.image.load(rsrc("image_race/board.png")).convert_alpha(),
        pygame.image.load(rsrc("image_race/yashi.png")).convert_alpha(),
        pygame.image.load(rsrc("image_race/yacht.png")).convert_alpha()
    ]
    img_car = [
        pygame.image.load(rsrc("image_race/car00.png")).convert_alpha(),
        pygame.image.load(rsrc("image_race/car01.png")).convert_alpha(),
        pygame.image.load(rsrc("image_race/car02.png")).convert_alpha(),
        pygame.image.load(rsrc("image_race/car03.png")).convert_alpha(),
        pygame.image.load(rsrc("image_race/car04.png")).convert_alpha(),
        pygame.image.load(rsrc("image_race/car05.png")).convert_alpha(),
        pygame.image.load(rsrc("image_race/car06.png")).convert_alpha(),
        pygame.image.load(rsrc("image_race/car10.png")).convert_alpha(),
        pygame.image.load(rsrc("image_race/car11.png")).convert_alpha(),
        pygame.image.load(rsrc("image_race/car12.png")).convert_alpha(),
        pygame.image.load(rsrc("image_race/car13.png")).convert_alpha(),
        pygame.image.load(rsrc("image_race/car14.png")).convert_alpha(),
        pygame.image.load(rsrc("image_race/car15.png")).convert_alpha(),
        pygame.image.load(rsrc("image_race/car16.png")).convert_alpha(),
        pygame.image.load(rsrc("image_race/car20.png")).convert_alpha(),
        pygame.image.load(rsrc("image_race/car21.png")).convert_alpha(),
        pygame.image.load(rsrc("image_race/car22.png")).convert_alpha(),
        pygame.image.load(rsrc("image_race/car23.png")).convert_alpha(),
        pygame.image.load(rsrc("image_race/car24.png")).convert_alpha(),
        pygame.image.load(rsrc("image_race/car25.png")).convert_alpha(),
        pygame.image.load(rsrc("image_race/car26.png")).convert_alpha()
    ]
    se_crash = pygame.mixer.Sound(rsrc("sound_race/crash.ogg"))
    BOARD_W = [0] * BOARD
    BOARD_H = [0] * BOARD
    BOARD_UD = [0] * BOARD
    ...
    init_car()
    vertical = 0
    game_ost = [pygame.mixer.Sound(rsrc("sound_race/bgm.ogg"))]
    ...
    while True:
        ...
        if idx == 0:
            ...
            draw_text(screen, "[s] Start game", 400, 320, WHITE, fnt_m)
            draw_text(screen, "[c] choose your car", 400, 400, WHITE, fnt_m)
            move_car(0)
            if key[K_s] != 0:
                init_car()
                idx = 1
        ...
        if idx == 3:
            ...
            if tmr == 30:
                pygame.mixer.music.load(rsrc("sound_race/goal.ogg"))
            ...
            if tmr >= 230:
                ...
                def send_to_server(name):
                    try:
                        ...
                        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as sock:
                            ...
                            send_data = f"{name}," + ",".join(laptime) + f",{time_str(rec)}"
                            sock.sendall(send_data.encode('utf-8'))
                    except Exception as e:
                        messagebox.showerror("서버 오류", f"서버 전송 실패: {e}")
                def on_submit():
                    name = entry_name.get()
                    if not name:
                    ...
                    root.destroy()
                    import sys
                    launcher_path = os.path.join(os.path.dirname(__file__), '../../my_launcher.py')
                ...
                title = tk.Label(root, text="기록 입력", font=("malgungothic", int(screen_height*0.05), "bold"), fg="#00bfff", bg="#1e1e1e")
        ...
        if idx == 4:
            ...
            if key[K_RETURN] == 1:
                idx = 0
        # Add "Home 'M'" text in the bottom right corner
        font = pygame.font.Font(None, 28)
        text = 'Home "M"'
        txt_surface = font.render(text, True, (40, 40, 40))
        screen.blit(txt_surface, (800 - txt_surface.get_width() - 18, 600 - txt_surface.get_height() - 12))
        txt_surface = font.render(text, True, (255, 255, 255))
        screen.blit(txt_surface, (800 - txt_surface.get_width() - 20, 600 - txt_surface.get_height() - 14))
        pygame.display.update()
        clock.tick(60)
